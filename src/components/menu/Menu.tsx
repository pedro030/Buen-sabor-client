...
const Menu = () => {
  ...
  // Filters
  const { filters, setFilters, filterProducts }: IFilterContext =
  ...
  useEffect(() => {
    const getAndSetData = async () => {
      ...
      await fetch(`${apiUrl}/categories/getAll`, {
        ...
        .then((res) => res.json())
        .then((data) => {
          const cat = data.filter((c: MCategory) => c.parentCategory != null);
          setCategories(cat);
        })
        .catch((error) => console.error(error));
    };
  ...
  const sortProducts = (products: MProduct[], sortOp: number) => {
    switch (sortOp) {
      ...
      case 2:
        setSortedProducts(
          products.sort((a: MProduct, b: MProduct) =>
            a.price > b.price ? 1 : -1
          )
        );
        break;
    ...
    <>
      <div className='p-8'>
        ...
        <div className='flex flex-col lg:flex-row'>
          ...
          {/* FilterButton */}
          <div className='w-full mb-3 dropdown lg:hidden'>
            <label tabIndex={0} className='w-full mb-1 btn btn-primary btn-sm'>
              Filters
            </label>
            <ul
              tabIndex={0}
            ...
            >
              <div className='filter'>
                ...
                <div className='flex items-center justify-between '>
                  ...
                  <img
                    className='items-center h-4 cursor-pointer'
                    src={clean}
                    onClick={() => {
                      setFilters({
                        category: "all",
                ...
                <form className='pt-2 form-control'>
                  ...
                  <div>
                    <input
                      type='radio'
                      name='category'
                      className='w-4 h-4 mr-1 rounded checkbox checkbox-primary'
                      value='all'
                      onChange={handleChangeCategory}
                      checked={filters.category == "all" ? true : false}
                      readOnly
                    />
                    <label className='label-text'>Todos</label>
                    <br />
                      ...
                      return (
                        <span key={i}>
                          <input
                            ...
                            onChange={handleChangeCategory}
                          />
                          <label className='label-text'>{c.name}</label>
                          <br />
                        </span>
                      );
                    })}
                  ...
                  <div className='flex flex-col gap-3'>
                    <div className='flex flex-row items-center justify-between'>
                      <label className='mr-2 text-xs label-text'>
                        Min Price:
                      </label>
                      <input
                        type='number'
                        className='w-14 input input-bordered input-xs'
                        min={0}
                    ...
                    <div className='flex flex-row items-center justify-between'>
                      ...
                      <input
                        ...
                        value={
                          filters.maxPrice == 20000 ? "" : filters.maxPrice
                        }
                      />
                    </div>
                  </div>
          ...
          {/* FilterMenu */}
          <div className='hidden filter lg:inline'>
            {/* RESET FILTERS */}
            <div className='flex items-center justify-between '>
              <h2 className='card-title stat-title'>Filter</h2>
              <img
          ...
          {/* Menu */}
          <div className='flex-1 products'>
            <div className='flex flex-col items-center w-full gap-5 sm:items-start sm:justify-between sm:flex-row'>
            ...
            <div className='flex justify-end'>
              {products.length > 0 && (
                <div className='mt-5 join '>
                  <button
                    className='join-item btn btn-sm max-lg:btn-xs'
                    onClick={() =>
                  ...
                  {pages.map((page: number, index: number) => {
                    return (
                      <input
                        key={index.toString()}
                        className='join-item btn btn-sm max-lg:btn-xs btn-square'
                        type='radio'
                        name='options'
          ...
          {/* Cart */}
          <div className='hidden min-w-[14rem] xl:min-w-[18rem]  order lg:inline'>
            <div className='flex flex-col items-end mt-12 max-lg:mt-10'>
              <div className='w-full px-4 bg-white rounded-xl'>
                    ...
                    {cart[0].quantity != 0
                      ? cart.map((item: MCart) => {
                          return (
                            <div
                              key={item.product.id}
                              className='pr-2 text-xs product_order'
                            >
                              <h4>
                ...
                <div>
                  <h4 className='text-right max-lg:text-sm'>
                    subtotal:{" "}
                  ...
                  <div className='flex justify-center mt-2'>
                    <button
                      className={`w-full mb-2 rounded-full btn btn-primary ${
